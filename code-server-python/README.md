...
A Helm chart for deploying a Python-based code server for internal use.
...
```bash
helm install code-server-python ./code-server-python
```

### Uninstall the chart

```bash
helm uninstall code-server-python
```

## Configuration

The following table lists the configurable parameters of the code-server-python chart and their default values.
...
```bash
helm install code-server-python ./code-server-python -f values.yaml
```
